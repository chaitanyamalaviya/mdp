...
public class PathSolver {
	...
	private Stack<Point> startPoints = new Stack<Point>();
	private Stack<Point> goalPoints = new Stack<Point>();
	
	public void initGoalPoints(){
		goalPoints.push(new Point(2,13));
		goalPoints.push(new Point(2,14));
		goalPoints.push(new Point(3,13));
		goalPoints.push(new Point(3,14));
	}
	
	public void initStartPoints(){
		goalPoints.push(new Point(18,2));
		goalPoints.push(new Point(19,2));
		goalPoints.push(new Point(18,3));
		goalPoints.push(new Point(19,3));
	}
	...
	public boolean checkUnexploredNorth(Point location,Maze maze){
		if(startPoints.contains(location)||goalPoints.contains(location))
			return false;
	...
	public Stack<Point> solvePath(Point root,Point goal,Maze maze,int ori){
		...
		if(cost1<cost2){
			if(cost3<cost4){
				if(cost1<cost3 && !path1.empty())
					return path1;
				else
					return path3;
			}
			else{
				if(cost1<cost4 && !path1.empty())
					return path1;
				else
					return path4;
			}
			...
			if(cost3<cost4){
				if(cost2<cost3 && !path2.empty())
					return path2;
				else
					return path3;
			}
			else{
				if(cost2<cost4 && !path2.empty())
					return path2;
				else
					return path4;
			}
	...
	public void moveRobot(Stack<Point> path,Point current,Point goal,Maze maze, int ori){
		Point tempPoint = new Point();
		String arduinoCommand = new String();
		//RaspberryController rc = new RaspberryController();
		...
		//ending point
		//rc.sendMessage("PS:"+arduinoCommand+"ST");
	}
}